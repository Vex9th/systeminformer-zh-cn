...
| 动态文字 | PE Viewer、安装器和主程序分别已有 128、74、271 条使用原生 `STRINGTABLE`；ExtendedServices、ExtendedTools、ToolStatus、UserNotes 分别已迁移 15、25、103、15 条，HardwareDevices、NetworkTools、OnlineChecks、Updater、WindowExplorer 合计迁移 13 条。扩展审计到窗口文字、组合框（含数组批量添加）及列表分组/项目和包装入口，并按真实翻译入口区分必须迁移的调用点后，当前报告 661 条未翻译项；大量既有文字仍由兼容翻译层接管，不能据此宣称整体完成 |
| 生成器与源码契约 | 本地测试通过 |
