...
- 不改变程序功能、权限模型、进程操作逻辑与系统访问行为
- 社区版产品标识为 **sys_info**：可执行文件名（`sys_info.exe`）、窗口标题、主窗口类名、临时服务名、转储文件前缀与版本资源产品名均为 sys_info，可与官方 System Informer 并存互不干扰；插件按新模块名链接，直接改官方版文件名会导致插件加载失败，请使用本社区版构建

## 功能说明（与上游一致）

本社区版只改界面本地化，不修改核心功能，常用功能与上游保持一致：进程/服务/网络/磁盘/句柄/模块/线程、系统资源图表、便携免安装运行。

## 截图

真实 Windows 截图尚未补齐（主窗口、进程属性、设置页）。当前先以官方截图与文字说明对齐。
...
3. 解压到任意目录运行 `amd64\sys_info.exe`（ARM64 设备用 `arm64\`，32 位系统用 `i386\`）
4. 想恢复英文界面：把设置文件（`sys_info.exe.settings.json`，便携模式放在程序目录）中的 `"Language": "zh-CN"` 改为 `"en"` 后重启程序；未翻译的字符串也会自动显示英文

> 程序未做代码签名，SmartScreen 可能弹出警告；请核对 SHA-256 后选择"仍要运行"，或自行从源码构建。

## 无驱动说明

- 仅构建用户态程序，不打包 `KSystemInformer`，产物不含 `.sys`/`.inf`，也不安装内核驱动服务。
- 依赖驱动的高级功能不可用（受保护进程完整句柄枚举、内核态堆栈采集等）将回退或显示不可用。
- CI 会在打包前递归检查，发现 `.sys`/`.inf` 立即失败。
...
这两个标签**不是规则/历史列表，而是实时事件监视器**，行为与官方版一致：

- **磁盘（Disk）**：以管理员身份运行后，实时显示各进程的磁盘读写活动；未提权时空列表并提示需要管理员权限
- **防火墙（Firewall）**：以管理员身份运行后，实时显示被 Windows 防火墙**拦截的网络连接事件**；没有发生拦截时列表为空，这是正常现象，不代表功能损坏。若提权后仍显示"无法启动防火墙事件跟踪会话"，请把错误代码反馈到 Issues
