...
进行中（2026-09-06）：审计范围已覆盖主程序、插件、`peview`、`CustomSetupTool` 和 `CustomSignTool`。当前共 729 条 en-US/zh-CN `STRINGTABLE` 项：PE Viewer 128 条，安装器 74 条，主程序 290 条，DotNetTools 8 条，ExtendedServices 66 条，ExtendedTools 26 条，ToolStatus 103 条，UserNotes 15 条，HardwareDevices、NetworkTools、OnlineChecks、Updater、WindowExplorer 合计 19 条；PE Viewer 的菜单、列表列名、选项和弹窗提示，以及主程序选项/启动/公共错误提示、内置插件名称与说明、插件管理器动态标签、常用下拉选项、服务控制按钮、远程控制热键和无共享冲突的列表分组、DotNetTools 的 8 个 CLR 性能计数器分组，以及 WindowExplorer 的 UI Automation 分组、ExtendedTools/Updater 的编辑器和更新间隔选项、ExtendedServices 的 SID、服务保护、恢复动作、触发器类型/子类型/动作、依赖项/依赖者状态及 PnP 状态文字已迁移，并新增扫描式守卫防止英文提示回流。触发器、服务恢复和远程控制热键编辑器不再按显示文字反向解析业务值；普通字符串数组以及带限定符、可选类型转换和 `SIP(...)` 初始化的结构体字段数组传入组合框的模式也已纳入审计。该漏检曾新增暴露的 57 条组合框文字现已全部迁移；ExtendedServices 当前审计项已全部覆盖，没有运行时翻译 hook 的窗口文字、组合框和分组列表项按必须迁移调用点计数，项目仍如实报告 623 条未翻译项。旧字典仍在接管大量既有文字；任务 8 及后续兼容层移除、Windows 编译与真实界面验收仍未完成。
