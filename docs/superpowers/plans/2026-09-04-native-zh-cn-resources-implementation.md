...
**当前状态：** 方案 B 尚未完成。主程序静态对话框和语言感知加载路径已落地；插件、独立工具、动态字符串、旧运行时翻译层移除、x86/ARM64 与多 DPI 验收仍按下列任务推进。
...
- [ ] **步骤 1：盘点每个工具的可见资源和实际发布范围**
- [ ] **步骤 2：生成并接入同 ID 的 zh-CN 资源**
