...
**当前状态：** 方案 B 尚未完成。主程序、11 个插件和独立工具的静态对话框以及语言感知加载路径已在源码落地；PE Viewer 的菜单、列表列名、选项和弹窗提示，以及安装器向导文字已迁入原生字符串表。其余动态字符串、旧运行时翻译层移除、Windows CI、x86/ARM64 与多 DPI 验收仍按下列任务推进。
...
进行中（2026-09-05）：审计范围已覆盖主程序、插件、`peview`、`CustomSetupTool` 和 `CustomSignTool`。工具当前共 202 条 en-US/zh-CN `STRINGTABLE` 项：PE Viewer 128 条，安装器 74 条；PE Viewer 的菜单、列表列名、选项和弹窗提示已迁移，并新增扫描式守卫禁止 `PhShow*` 调用重新引入英文提示。PE Viewer 其余动态字段、主程序和插件动态文字仍使用旧兼容字典，因此任务 8 仍不能勾选完成。
