...
这里放本地化数据、生成器和验证脚本。方案 B 仍在迁移中：当前源码已为主程序、11 个插件、PE Viewer 和安装器生成原生 zh-CN 对话框；已有 224 条动态文字迁到 `STRINGTABLE`，其中 PE Viewer 128 条、安装器 74 条、主程序选项/启动提示 22 条。其余动态文字仍依赖旧兼容翻译层，所以当前审计的未翻译条目为 0 也不能说明整体完成。
