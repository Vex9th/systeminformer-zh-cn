...
这里放本地化数据、生成器和验证脚本。方案 B 仍在迁移中：当前源码已为主程序、11 个插件、PE 查看器和安装器生成原生 zh-CN 对话框；安装器实际向导的 41 条动态文字已迁到 `STRINGTABLE`。其余动态文字仍依赖旧兼容翻译层，最新审计明确列出 115 条未迁移项，因此不能据此宣称整体完成。
