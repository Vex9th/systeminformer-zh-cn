...
class ExtendedToolsGroupItemResourceTests(unittest.TestCase):
    ...
    def test_resources_have_exact_ids_text_layers_and_boundaries(self) -> None:
        ...
        self.assertRegex(header, r"(?m)^#define\s+_APS_NEXT_SYMED_VALUE\s+61397$")
        self.assertEqual(len(re.findall(r'(?m)^\s*IDS_ET_[A-Z0-9_]+\s+"', english_rc)), 397)
        self.assertEqual(len(re.findall(r'(?m)^\s*IDS_ET_[A-Z0-9_]+\s+"', chinese_rc)), 397)
    ...
    def test_ci_requires_all_188_extended_tools_strings(self) -> None:
        ...
        self.assertEqual(
            workflow.count("bin\\Release64\\plugins\\ExtendedTools.dll=397"),
            2,
        )
