...
class ExtendedToolsGroupItemResourceTests(unittest.TestCase):
    ...
    def test_resources_have_exact_ids_text_layers_and_boundaries(self) -> None:
        ...
        self.assertEqual(len(re.findall(r'(?m)^\s*IDS_ET_[A-Z0-9_]+\s+"', english_rc)), 470)
        self.assertEqual(len(re.findall(r'(?m)^\s*IDS_ET_[A-Z0-9_]+\s+"', chinese_rc)), 470)
    ...
    def test_ci_requires_all_188_extended_tools_strings(self) -> None:
        ...
        self.assertEqual(
            workflow.count("bin\\Release64\\plugins\\ExtendedTools.dll=470"),
            2,
        )
