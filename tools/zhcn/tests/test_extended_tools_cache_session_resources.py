...
class ExtendedToolsCacheSessionResourceTests(unittest.TestCase):
    ...
    def test_cached_range_aliases_cover_every_loaded_resource(self):
        ...
        self.assertRegex(header, r"(?m)^#define IDS_ET_CACHED_LAST\s+IDS_ET_DRAM_LATENCY_SUMMARY_FORMAT$")
        self.assertRegex(
            main,
    ...
    def test_resources_json_ci_and_generator_boundaries_are_exact(self):
        ...
        self.assertEqual(sorted(defines.values()), list(range(61000, 61132)))
        self.assertEqual(set(defines), set(english))
        self.assertEqual(set(defines), set(chinese))
        self.assertEqual(len(english), 132)
        self.assertEqual(len(chinese), 132)
        self.assertRegex(header, r"(?m)^#define _APS_NEXT_SYMED_VALUE\s+61132$")

        workflow = (REPO_ROOT / ".github" / "workflows" / "zh-cn-build.yml").read_text(
            encoding="utf-8"
        )
        self.assertEqual(workflow.count("plugins\\ExtendedTools.dll=132"), 2)
        self.assertNotIn("plugins\\ExtendedTools.dll=116", workflow)

        generator_test = (
            REPO_ROOT / "tools" / "zhcn" / "tests" / "test_native_resource_generation.py"
        ).read_text(encoding="utf-8")
        self.assertIn('self.assertIn("1505 strings", result.stdout)', generator_test)
