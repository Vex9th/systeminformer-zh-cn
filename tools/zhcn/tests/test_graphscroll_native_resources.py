...
class GraphScrollNativeResourceTests(unittest.TestCase):
    ...
    def test_shared_ids_resources_translation_ownership_and_counts_are_exact(self) -> None:
        ...
        self.assertEqual(len(system_en), 663)
        self.assertEqual(len(system_zh), 663)
        self.assertEqual(len(peview_en), 311)
        self.assertEqual(len(peview_zh), 311)
        self.assertRegex(system_header, r"(?m)^#define\s+IDS_PH_LAST\s+IDS_PH_HANDLE_SECTION_RESERVE$")
        self.assertRegex(system_header, r"(?m)^#define\s+_APS_NEXT_SYMED_VALUE\s+2663$")
    ...
    def test_runtime_entries_are_retired_and_generated_count_is_exact(self) -> None:
        ...
        self.assertIn("2348 entries", result.stdout)
    ...
    def test_ci_and_native_generator_counts_are_exact(self) -> None:
        ...
        self.assertEqual(workflow.count(r"sys_info.exe=663"), 2)
        ...
        self.assertIn('self.assertIn("1815 strings", result.stdout)', generator_test)
