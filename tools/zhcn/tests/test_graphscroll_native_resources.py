...
class GraphScrollNativeResourceTests(unittest.TestCase):
    ...
    def test_shared_ids_resources_translation_ownership_and_counts_are_exact(self) -> None:
        ...
        self.assertEqual(len(system_en), 944)
        self.assertEqual(len(system_zh), 944)
        self.assertEqual(len(peview_en), 311)
        self.assertEqual(len(peview_zh), 311)
        self.assertRegex(system_header, r"(?m)^#define\s+IDS_PH_LAST\s+IDS_PH_TIMELINE$")
        self.assertRegex(system_header, r"(?m)^#define\s+_APS_NEXT_SYMED_VALUE\s+2944$")
    ...
    def test_runtime_entries_are_retired_and_generated_count_is_exact(self) -> None:
        ...
        self.assertIn("2252 entries", result.stdout)
    ...
    def test_ci_and_native_generator_counts_are_exact(self) -> None:
        ...
        self.assertEqual(workflow.count(r"sys_info.exe=944"), 2)
        ...
        self.assertIn('self.assertIn("2584 strings", result.stdout)', generator_test)
