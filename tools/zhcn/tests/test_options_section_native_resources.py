...
class OptionsSectionNativeResourceTests(unittest.TestCase):
    ...
    def test_all_twenty_resources_have_exact_native_ownership(self) -> None:
        ...
        self.assertRegex(app_header, r"(?m)^#define\s+IDS_PH_LAST\s+IDS_PH_MENU_VIRTUALIZATION$")
        self.assertRegex(app_header, r"(?m)^#define\s+_APS_NEXT_SYMED_VALUE\s+3051$")
    ...
    def test_exact_ci_and_generator_counts(self) -> None:
        ...
        self.assertIn("2691 strings", native.stdout)
        ...
        self.assertIn("2124 entries", runtime.stdout)
