...
class NativeResourceGenerationTests(unittest.TestCase):
    ...
    def test_main_tray_notification_items_use_native_resources(self) -> None:
        ...
        self.assertRegex(
            resource_header,
            r"(?m)^#define\s+IDS_PH_LAST\s+IDS_PH_GROUP_PACKAGE$",
        )
        self.assertRegex(
            resource_header,
            r"(?m)^#define\s+_APS_NEXT_SYMED_VALUE\s+2511$",
        )
    ...
    def test_all_generated_resources_are_current(self) -> None:
        ...
        self.assertIn("1472 strings", result.stdout)
    ...
    def test_peview_context_menus_use_native_string_resources(self) -> None:
        ...
        self.assertEqual(len(stringtable_ids(resource_script)), 270)
    ...
    def test_peview_hash_groups_use_native_string_resources(self) -> None:
        ...
        self.assertRegex(
            resource_header,
            r"(?m)^#define\s+IDS_PV_LAST\s+IDS_PV_SUBSYSTEM_WINDOWS_BOOT_APPLICATION$",
        )
        self.assertRegex(
            resource_header,
            r"(?m)^#define\s+_APS_NEXT_SYMED_VALUE\s+3270$",
        )
        self.assertEqual(len(stringtable_ids(english_rc)), 270)
        self.assertEqual(len(stringtable_ids(chinese_rc)), 270)
    ...
    def test_main_options_messages_use_native_string_resources(self) -> None:
        ...
        self.assertEqual(len(stringtable_ids(resource_script)), 511)
        self.assertIn(
        ...
        self.assertEqual(sorted(numeric_ids), list(range(2000, 2511)))
    ...
    def test_extended_tools_adapter_details_share_native_resources(self) -> None:
        ...
        self.assertRegex(
            resource_header,
            r"(?m)^#define\s+_APS_NEXT_SYMED_VALUE\s+61116$",
        )
            ...
            Counter(
                {
                    ...
                    (r"bin\Release64\peview.exe", 270): 2,
