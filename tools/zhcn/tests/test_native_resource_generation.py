...
class NativeResourceGenerationTests(unittest.TestCase):
    ...
    def test_main_tray_notification_items_use_native_resources(self) -> None:
        ...
        self.assertRegex(
            resource_header,
            r"(?m)^#define\s+IDS_PH_LAST\s+IDS_PH_GROUP_AUDITING_INFORMATION$",
        )
        self.assertRegex(
            resource_header,
            r"(?m)^#define\s+_APS_NEXT_SYMED_VALUE\s+2488$",
        )
    ...
    def test_all_generated_resources_are_current(self) -> None:
        ...
        self.assertIn("1401 strings", result.stdout)
    ...
    def test_peview_hash_groups_use_native_string_resources(self) -> None:
        ...
        self.assertRegex(
            resource_header,
            r"(?m)^#define\s+IDS_PV_LAST\s+IDS_PV_GROUP_INTERNAL_INFORMATION$",
        )
        self.assertRegex(
            resource_header,
            r"(?m)^#define\s+_APS_NEXT_SYMED_VALUE\s+3239$",
        )
        self.assertEqual(len(stringtable_ids(english_rc)), 239)
        self.assertEqual(len(stringtable_ids(chinese_rc)), 239)
    ...
    def test_main_options_messages_use_native_string_resources(self) -> None:
        ...
        self.assertEqual(len(stringtable_ids(resource_script)), 488)
        self.assertIn(
        ...
        self.assertEqual(sorted(numeric_ids), list(range(2000, 2488)))
    ...
    def test_ci_validates_all_built_plugin_resources(self) -> None:
        ...
        self.assertEqual(
            ...
            Counter(
                {
                    (r"bin\Release64\sys_info.exe", 488): 2,
                    ...
                    (r"bin\Release64\peview.exe", 239): 2,
