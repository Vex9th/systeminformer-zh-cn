...
class NativeResourceGenerationTests(unittest.TestCase):
    ...
    def test_peview_context_menus_use_native_string_resources(self) -> None:
        ...
        self.assertEqual(len(stringtable_ids(resource_script)), 247)
    ...
    def test_peview_hash_groups_use_native_string_resources(self) -> None:
        ...
        self.assertRegex(
            resource_header,
            r"(?m)^#define\s+IDS_PV_LAST\s+IDS_PV_VERIFYING_COMPANY$",
        )
        self.assertRegex(
            resource_header,
            r"(?m)^#define\s+_APS_NEXT_SYMED_VALUE\s+3247$",
        )
        self.assertEqual(len(stringtable_ids(english_rc)), 247)
        self.assertEqual(len(stringtable_ids(chinese_rc)), 247)
            ...
            Counter(
                {
                    ...
                    (r"bin\Release64\peview.exe", 247): 2,
