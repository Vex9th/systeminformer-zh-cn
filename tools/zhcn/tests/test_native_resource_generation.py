...
class NativeResourceGenerationTests(unittest.TestCase):
    ...
    def test_main_tray_notification_items_use_native_resources(self) -> None:
        ...
        self.assertRegex(
            resource_header,
            r"(?m)^#define\s+IDS_PH_LAST\s+IDS_PH_TREENEW_WINDOW_TITLE$",
        )
        self.assertRegex(
            resource_header,
            r"(?m)^#define\s+_APS_NEXT_SYMED_VALUE\s+3296$",
        )
    ...
    def test_main_options_messages_use_native_string_resources(self) -> None:
        ...
        self.assertEqual(len(stringtable_ids(resource_script)), 1296)
        self.assertIn(
