    ...
    )
{
    if ((HWND)wParam == ListViewHandle)
    {
        ...
        if (PhGetSelectedListViewItemParams(ListViewHandle, &listviewItems, &numberOfItems))
        {
            ...
            PhInsertEMenuItem(menu, PhCreateEMenuItem(0, USHRT_MAX, PvpLoadUiString(IDS_PV_MENU_COPY_ACCELERATED), NULL, NULL), ULONG_MAX);
    ...
    )
{
    ...
    sizeColumnToFitMenuItem = PhCreateEMenuItem(0, PH_TN_COLUMN_MENU_SIZE_COLUMN_TO_FIT_ID, PvpLoadUiString(IDS_PV_MENU_SIZE_COLUMN_TO_FIT), NULL, NULL);
    sizeAllColumnsToFitMenuItem = PhCreateEMenuItem(0, PH_TN_COLUMN_MENU_SIZE_ALL_COLUMNS_TO_FIT_ID, PvpLoadUiString(IDS_PV_MENU_SIZE_ALL_COLUMNS_TO_FIT), NULL, NULL);

    if (!(Flags & PH_TN_COLUMN_MENU_NO_VISIBILITY))
    {
        hideColumnMenuItem = PhCreateEMenuItem(0, PH_TN_COLUMN_MENU_HIDE_COLUMN_ID, PvpLoadUiString(IDS_PV_MENU_HIDE_COLUMN), NULL, NULL);
        chooseColumnsMenuItem = PhCreateEMenuItem(0, PH_TN_COLUMN_MENU_CHOOSE_COLUMNS_ID, PvpLoadUiString(IDS_PV_MENU_CHOOSE_COLUMNS), NULL, NULL);
    }
    ...
    if (Flags & PH_TN_COLUMN_MENU_SHOW_RESET_SORT)
    {
        ...
        if (sortOrder != Data->DefaultSortOrder || (Data->DefaultSortOrder != NoSortOrder && sortColumn != Data->DefaultSortColumn))
            resetSortMenuItem = PhCreateEMenuItem(0, PH_TN_COLUMN_MENU_RESET_SORT_ID, PvpLoadUiString(IDS_PV_MENU_RESET_SORT), NULL, NULL);
    }
